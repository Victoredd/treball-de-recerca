...
float sensorRead(int selectedSensor) {
  ...
  if (selectedSensor == 1) {
    // 7 is default oversampling rate for standard precision (2^7 samples)
    dps.measurePressureOnce(val, 7);
  } 
  else if (selectedSensor == 2) {
    VL53L0X_RangingMeasurementData_t measure;
    lox.rangingTest(&measure, false);
    if (measure.RangeStatus != 4) {  // not failed
      val = (float)measure.RangeMilliMeter;
    } else {
      val = 0.0; // Error or out of range
    }
  }
  return val;
}
